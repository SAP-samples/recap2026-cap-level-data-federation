# Preparation

## Note

This exercise description has been written for Windows computers.
You can of course run the exercise also on different machines, but you may have to adapt in a few places.


## Preparation 1 - Setup workspace

Choose a suitable folder on your machine for this exercise.

Once in this folder, clone the repo of this session, move into the _ws_ folder within it, and then open it with VS Code:
```sh
git clone https://github.com/SAP-samples/recap2026-cap-level-data-federation.git
cd recap2026-cap-level-data-federation/ws
code .
```

The _ws_ folder is where you will be developing the CAP applications.

You should see this folder structure:

<br>![](/exercises/ex0/images/00_01_0010.png)


## Preparation 2 - (optional) Activate auto-save

Each time you change a file in the course of this exercise, don't forget to save with `Ctrl+S` or via the "File" menu.
Alternatively, you can also activate the "Auto Save" feature of VS Code via the "File" menu.


## Summary

You have now prepared the workspace.

Continue to [Exercise 1 - Create API package for xflights](../ex1/README.md).
