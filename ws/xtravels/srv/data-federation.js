...
PROD || cds.once ('served', () => Promise.all (feed.map (async each => {
  ...
  // .as(each.entity) gives each scheduled task a unique name so the queue assigns
  // distinct deterministic IDs (based on service+task). Without it, all entities
  // sharing the same remote service get the same ID and UPSERT into the task table
  // overwrites each other, leaving only one entity scheduled.
